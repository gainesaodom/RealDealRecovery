...
#define SPI_MEM_DEVICE2 "/dev/spidev0.1" // we're going to open SPI on bus 0 device 1
...
void spi_mem_init2(uint64_t speed) {
	...
	// open device
    fd2 = open(SPI_MEM_DEVICE2, O_RDWR);
	if (fd2 < 0) {
		printf("Could not open\n");
		exit(EXIT_FAILURE);
	}
	...
	//uint32_t spi_speed_hz = SPI_SPEED_HZ;
	// assign the speed of ioctl
	ret = ioctl(fd2, SPI_IOC_WR_MAX_SPEED_HZ, &spi_mem_speed_hz);
	if (ret != 0) {
		printf("Could not write the SPI max speed...\r\n");
		close(fd2);
		exit(EXIT_FAILURE);
	...
	ret = ioctl(fd2, SPI_IOC_RD_MAX_SPEED_HZ, &spi_mem_speed_hz);
	if (ret != 0) {
		printf("Could not read the SPI max speed...\r\n");
		close(fd2);
		exit(EXIT_FAILURE);
...
uint8_t spi_mem_read_byte(uint32_t addr) {
    ...
    tx_buffer[2] = (uint8_t) (addr >> 8) & 0xFF; // middle third of address
